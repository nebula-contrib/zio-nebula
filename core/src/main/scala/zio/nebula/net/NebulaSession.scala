 ...
 */
final class NebulaSession(underlying: Session) {

  def execute(stmt: String): Task[NebulaResultSet] = ZIO.attempt(new NebulaResultSet(underlying.execute(stmt)))

  def executeWithParameter(stmt: String, parameterMap: Map[String, Any]): Task[NebulaResultSet] =
  ...
  def executeJsonWithParameter(stmt: String, parameterMap: Map[String, Any]): Task[String] =
